...
class Dashboard:
    ...
    def draw_dashboard(self):
        ...
        try:
            output = float(self.output_value)
        except Exception:
            output = 0.0
        try:
        ...
        if target > 0:
            percent = round(output / target * 100, 2)
        else:
            percent = 0.0
        ...
        for i, hour in enumerate(range(8, 23)):
            ...
            # if pcs is not 0:
            if pcs != 0 and target_hr > 0:
                # target = int(self.target_value) if str(self.target_value).isdigit() else 0
                percent = (pcs / target_hr) * 100
        ...
        try:
            man_plan = int(self.man_plan)
        except Exception:
            man_plan = 0
        try:
            man_act = int(self.man_act)
        except Exception:
            man_act = 0

        if man_plan > 0:
            productivity_plan = int(self.target_value) / man_plan
        else:
            productivity_plan = 0.0

        if man_act > 0 and num_hours > 0:
            productivity_act = int(self.output_value) / man_act / num_hours
        else:
